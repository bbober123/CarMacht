(function () {
  ...
  supabaseClient.auth.onAuthStateChange(function (event, session) {
    if (session) {
      if (window.location.search.includes("popup=1") && window.opener) {
  ...
  document.getElementById("google-btn").addEventListener("click", async function () {
    ...
    var res = await supabaseClient.auth.signInWithOAuth({
      provider: "google",
      options: { 
        redirectTo: window.location.origin + window.location.pathname.replace(/[^/]*$/, "") + "auth.html?popup=1",
        skipBrowserRedirect: true
      },
    });
    if (res.error) {
      statusEl.textContent = T("common.error_prefix") + res.error.message;
    } else if (res.data && res.data.url) {
      window.open(res.data.url, 'oauth_popup', 'width=600,height=700');
      statusEl.textContent = "Logowanie w nowym oknie...";
    }
  });
